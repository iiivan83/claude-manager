...
Форматирует номер сессии как кликабельную ссылку — для сообщений из чужих сессий. При нажатии на ссылку в Telegram в поле ввода подставляется `/номер`.

**Аргументы:**
- `session_number` (int) — дневной номер сессии

**Возвращает:** HTML-строку с кликабельной ссылкой (например, `"<b><a href=\"tg://msg?text=/3\">#3</a></b>"`)
...
- **test_format_clickable_session_number** — проверяет формат кликабельного номера сессии
  ...
  - Ожидаемый результат: строка содержит `<a href=` и `/3` и `#3`
  - Тип: unit
