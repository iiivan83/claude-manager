...
│   ├── process_types.py      # Общие типы и результаты process lifecycle: SendResult, StopResult, callbacks
...
- **Слой инфраструктуры** (process_manager facade, process_* модули, process_state, claude_runner) — запуск процессов, хранение in-memory state процессов, чтение stdout, retry, `/stop`, протокол stream-json. Не знает про Telegram.
...
- **Принцип владения ходом (pause_owner_token).** Обработка одного сообщения — «ход» (`claude_interaction.send_to_claude_and_respond`) — на старте ставит паузу наблюдателя, watchdog тишины и флаг `handler_owns_final_delivery`. Второе сообщение той же сессии может стартовать в окне после снятия busy-флага, но до доставки финала/summary первого хода (до 45 сек, `SESSION_SUMMARY_TIMEOUT_SECONDS`), и завладеть паузой. Поэтому `finally` первого хода НЕ снимает паузу/watchdog безусловно: каждый ход создаёт уникальный токен (`SessionWatcherState.pause_owner_token`), передаёт его в `pause_session(owner_token=...)`, а `finally` снимает паузу/watchdog только внутри `session_watcher.is_pause_owner(session_id, backend, turn_pause_token)` — иначе снёс бы паузу/watchdog второго хода (P2-20). На путях `/stop`/ошибки, где обработчик финал не доставит (локальный флаг `final_answer_delivered_to_user` остался False), `finally` дополнительно вызывает `clear_handler_owns_final_delivery`, иначе флаг зависает навсегда и watcher придержит будущий внешний финал (P2-19). `reset_watchdog_on_progress` переставляет паузу тем же токеном; токен переживает ремап `session_id` (temp→real) через `update_session_id`
...
- **Silence mode (режим тишины).** Текстовые команды `Silence on` / `Silence off` (регистронезависимые) управляют глобальным режимом подавления промежуточных сообщений. Когда silence mode включён — бот доставляет только финальные ответы Claude (`is_final=True`), подавляя промежуточные обновления (thinking, progress) как в `telegram_response_delivery.send_response`, так и в `telegram_response_delivery.send_watcher_message`. Команды перехватываются в `telegram_input_handlers.handle_message` ПОСЛЕ `_check_access` и ДО `is_monitoring_mode` — работают в любом состоянии. Состояние персистентно через модуль `silence_mode_registry` (файл `~/.claude-manager-silence-mode`, паттерн `_loaded_from_disk` + атомарная запись). Режим глобальный — не сбрасывается при переключении проектов. Загрузка состояния выполняется в `telegram_lifecycle_handlers.post_init`
