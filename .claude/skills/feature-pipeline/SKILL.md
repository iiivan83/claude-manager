...
## Инвариант: делегирование обязательно

**Оркестратор делегирует ВСЕ фазы агентам через Agent tool. Самоисполнение — выполнение работы фазы напрямую вместо делегирования — запрещено АБСОЛЮТНО.**

Это правило не имеет исключений. «Оптимизация», «ускорение», «экономия контекста» и любые другие причины не являются основанием для самоисполнения. Если Agent tool недоступен (rate limit, quota) — эскалация пользователю (раздел 19 skills-agents-rules.md), а не переключение в режим «делаю сам».

Нарушение блокируется structural-validator.sh (check_orchestrator_discipline в critical-режиме).
...
5. Если `needs_rollback` = true → откат
5.5. **Запусти structural-validator.sh** с переменной `ORCHESTRATOR_LOG_PATH=<путь к orchestrator-log.json текущего прогона>`:
   ```bash
   ORCHESTRATOR_LOG_PATH="{LOG_DIR}/orchestrator-log.json" bash .claude/skills/feature-pipeline/scripts/structural-validator.sh
   ```
   Если валидатор вернул FAIL — остановись и сообщи пользователю. Коммит запрещён до исправления.
6. Если всё ОК:
...
## Обработка отказа инструмента делегирования

Полное правило — раздел 19 глобального референса `~/.claude/references/skills-agents-rules.md`. Проектные категории отказа CLI-подпроцесса — подраздел «Дисциплина оркестратора при отказе CLI-подпроцесса» в корневом `CLAUDE.md`. Не дублируй правило здесь — только ссылайся.

Категория «временная недоступность инструмента делегирования» (отказ Agent tool по лимиту квоты, отказ CLI-подпроцесса по `--max-budget-usd`, сетевой сбой, сбой preflight-проверки) принципиально отличается от категории «crash суб-агента» и обрабатывается отдельной процедурой из раздела 19. При отказе инструмента делегирования оркестратор `feature-pipeline` обязан остановиться и эскалировать пользователю — самостоятельный переход в режим «делаю сам» (выполнение работы фазы напрямую вместо делегирования) **запрещён абсолютно**, независимо от прежних договорённостей в сессии.
