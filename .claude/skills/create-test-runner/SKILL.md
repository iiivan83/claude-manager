...
description: >-
  ...
  Создаёт development/scripts/run-all-tests.sh — единый скрипт, который запускает
...
1. Проверь, существует ли уже файл `development/scripts/run-all-tests.sh`. Если да — предупреди пользователя, что файл будет перезаписан, и спроси подтверждение.
...
3. Убедись, что папка `development/scripts/` существует. Если нет — создай её.

## Шаг 2: Сгенерируй скрипт

**THINK HARD** — скрипт должен быть надёжным, понятным и правильно обрабатывать все случаи.

Создай файл `development/scripts/run-all-tests.sh` со следующим содержимым:
...
#
# Использование: ./development/scripts/run-all-tests.sh
...
```bash
chmod +x development/scripts/run-all-tests.sh
```
...
```bash
bash -n development/scripts/run-all-tests.sh
```
...
- **Создан:** development/scripts/run-all-tests.sh
- **Права:** исполняемый (chmod +x)
- **Синтаксис:** проверен (bash -n)
- **Запуск:** ./development/scripts/run-all-tests.sh
