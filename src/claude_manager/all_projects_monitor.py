...
async def _load_project_today_numbers(
    ...
    try:
        ...
        logger.warning(
            "Не удалось прочитать daily session registry проекта %s",
            project_path,
            exc_info=True,
        )
        return {}
    ...
    for project_number, project in enumerate(projects, start=1):
        ...
        for backend in coding_agent_backend.get_all_backends():
            try:
                ...
                logger.warning(
                    "Не удалось прочитать сессии проекта %s (%s)",
                    project.absolute_path,
                    backend.name.value,
                    exc_info=True,
                )
                continue
            sessions_with_backend.extend((file_info, backend) for file_info in files)
...
async def _build_baseline_states(
    ...
    for project_session in project_sessions:
        ...
        try:
            ...
            logger.warning(
                "Не удалось прочитать baseline сессии %s (%s)",
...
async def enable_for_chat(chat_id: int) -> None:
    ...
    try:
        ...
        raise
    logger.info("Чат %d включил глобальный all-режим", chat_id)
...
async def start(callback: AllProjectsMessageCallback) -> None:
    ...
    logger.info("Глобальный all-monitor запущен")
    try:
        while True:
            try:
                ...
                logger.error("Непредвиденная ошибка all-monitor", exc_info=True)
                await asyncio.sleep(ERROR_RETRY_DELAY_SECONDS)
                continue
            await asyncio.sleep(POLL_INTERVAL_SECONDS)
    except asyncio.CancelledError:
        logger.info("Глобальный all-monitor остановлен")
