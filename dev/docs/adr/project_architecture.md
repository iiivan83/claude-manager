...
**Принцип: сбой запуска CLI — отдельный тип исключения, не «занят».** `CodingAgentStartError` (подкласс `ProcessManagerError`) отделяет «CLI-процесс агента не стартовал» от «процесс уже занят предыдущим запросом». Все raise-сайты старта (`_start_subprocess_for_backend_turn`, Claude-ветка `_restart_process`, `create_process`) поднимают именно его; busy-гейт поднимает голый `ProcessManagerError`. Транспортный слой (`claude_interaction`, `reply_route_handler`) различает подтип и шлёт честное «не удалось запустить» вместо ложного «занят, подождите». Источник: ADR `05.07_01.41-...-coding-agent-start-error-vs-busy-diagnostics`, баг P2-15.

**Принцип: alias устаревшего temp-id резолвится в обеих формах ключа CLAUDE.** `_resolve_process_key_alias_unlocked` (через хелпер `_find_registered_alias_key`) находит alias и под голой строкой, и под tuple-ключом `(session_id, CLAUDE)`. Поэтому после temp→real ремапа `/stop`, `is_busy`, `has_process` и атомарный busy-гейт (`process_backend_send`) находят real-сессию по устаревшему temp-id, а не промахиваются мимо неё. Это возвращает код к уже описанному в CLAUDE.md контракту `_session_id_aliases`. Источник: баги P2-14, P2-17.
...
> Примечание: описание выше (счётчик `chat_active_requests`, `_pause_watcher`/`_resume_watcher`) отражает более ранний дизайн координации. Текущий код координирует паузу иначе — через `pause_session`/`resume_session` наблюдателя и владение ходом (см. следующий раздел). Полное согласование этого текста с актуальным кодом — отдельная задача устранения дрейфа документации (P3-57).

### Владение ходом: finally проверяет владение паузой и watchdog

Обработка одного пользовательского сообщения — «ход» (`claude_interaction.send_to_claude_and_respond`) — на старте ставит паузу наблюдателя (`session_watcher.pause_session`), запускает watchdog тишины stdout и помечает `handler_owns_final_delivery` (обработчик доставит финал сам, watcher его придерживает). В `finally` ход снимает паузу и watchdog.

Проблема: `finally` не должен снимать это состояние безоговорочно — им мог уже завладеть следующий ход той же сессии, либо флаг владения финалом мог протечь после `/stop`. Оба случая подтверждены как баги (P2-19, P2-20; ADR `05.07_02.26-...-turn-lifecycle-pause-ownership`).

Решение — **токен владения ходом** (`SessionWatcherState.pause_owner_token`):

- Каждый ход создаёт уникальный `object()` и передаёт его в `pause_session(..., owner_token=...)`. Второй ход перезаписывает токен своим.
- `SessionWatcher.is_pause_owner(session_id, token)` возвращает True, если состояния нет, паузой никто не владеет, либо владеет этот токен; False — только при чужом владельце.
- `finally` снимает watchdog и паузу только внутри `if is_pause_owner(...)`. Пауза/watchdog второго хода не сносятся (P2-20).
- Флаг `final_answer_delivered_to_user` ставится True только при реальной доставке через `handle_claude_result`. На путях `/stop`/ошибки он остаётся False, и `finally` вызывает `clear_handler_owns_final_delivery` — снимает зависший флаг (P2-19).
- `reset_watchdog_on_progress` переставляет паузу тем же токеном хода; токен переживает смену `session_id` (temp→real), потому что живёт в объекте состояния, переносимом через `update_session_id`.
