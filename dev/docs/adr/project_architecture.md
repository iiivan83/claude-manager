...
Бот принимает сообщения из Telegram, передаёт их в долгоживущий процесс Claude Code CLI, читает ответы из stdout и отправляет обратно в Telegram. Параллельно работает система мониторинга — watcher следит за файлами сессий на диске и уведомляет пользователя об активности в других сессиях.
...
### Утилиты — `message_splitter.py`, `session_reader.py`, `session_watcher.py`
...
- **session_watcher** — мониторинг файлов сессий в реальном времени, конвертация Markdown в HTML
...
Глобальное состояние (словари chat_id к значению):
- `chat_tools_mode` — режим инструментов для каждого чата
- `chat_working_dir` — рабочая папка для каждого чата
- `chat_watchers` — SessionWatcher для мониторинга одной сессии
- `chat_all_watchers` — AllSessionsWatcher для мониторинга всех сессий
- `chat_active_requests` — счётчик активных запросов (координация с watchers)
- `chat_session_epoch` — эпоха сессии (защита от перезаписи при переключении)
- `chat_monitor_only` — флаг режима наблюдения (сессия работает в терминале)
- `chat_pending_messages` — очередь сообщений, когда Claude занят
...
- `/all` — отключиться от сессии, слушать все через AllSessionsWatcher
...
### `session_watcher.py` — мониторинг в реальном времени

Два класса для наблюдения за файлами сессий:
...
- **AllSessionsWatcher** — следит за всеми `.jsonl` файлами в папке проекта, кроме текущей сессии. Отправляет короткие уведомления с deep link для подключения.
...
- `_pause_watcher()` — приостанавливает SessionWatcher и AllSessionsWatcher (увеличивает счётчик `chat_active_requests`, вызывает `pause()`)
- `_ensure_monitoring()` — гарантирует что запущен мониторинг текущей сессии
- Вызывает `_process_claude_request()` — основная логика
