...
**Ведущий `/` трактуется как slash-команда.** Если `message.content` начинается с символа `/`, Claude CLI разбирает первое слово как slash-команду ещё до вызова модели: для нераспознанного слова событие `result` содержит текст вида `Unknown command: /-new-things`. Так происходит, когда пользователь пересылает боту скопированное сообщение, начинающееся со слэша (заголовок ответа `/8 ...` или путь `/home/...`). Проверено эмпирически на реальном Claude CLI: ровно `/-new-things привет` → `Unknown command`, а ` /-new-things привет` (тот же текст с ведущим пробелом) уходит в модель как обычный текст. В Claude Manager это лечится в `src/claude_manager/process_send.py` (`_prevent_cli_slash_command_misparse`): перед отправкой в CLI текст, начинающийся с `/`, получает ведущий пробел — видимое содержимое не меняется.
