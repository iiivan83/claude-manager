...
- `is_error` (bool) — True, если ответ является ошибкой (ретраи исчерпаны либо ошибка постоянная — повтор не запускался)
- `retries_used` (int) — количество использованных повторных попыток (0, если ошибка постоянная и цикл повторов был пропущен)
- `permanent_error_kind` (PermanentErrorKind | None) — заполняется, если ошибка постоянная и повтор бессмыслен: `CONTEXT_OVERFLOW` (переполнение контекста) или `USAGE_LIMIT` (исчерпан лимит запросов). Транспортный слой по этому полю показывает понятное сообщение вместо «повтор N/10». None — ошибка временная или ответ успешный
...
- **Временная ошибка от Claude (is_error в result)** — Claude вернул ошибку, которую может исправить повтор (сбой API, обрыв соединения). Автоматический ретрай до 10 раз с интервалом 1 минута. Каждая попытка логируется на уровне warning. После исчерпания ретраев — логируется на уровне error, возвращается SendResult с is_error=True
- **Постоянная ошибка от Claude (is_error в result)** — ошибка, при которой повтор бессмыслен (переполнение контекста `Prompt is too long`, исчерпанный лимит `hit your limit`): `--resume` каждый раз грузит то же состояние. Backend-контракт классифицирует её через `classify_permanent_error`, а `_classify_permanent_error_result` собирает SendResult с `retries_used=0` и заполненным `permanent_error_kind` — цикл ретраев НЕ запускается. Логируется на уровне warning («повтор пропущен»). Транспортный слой по `permanent_error_kind` показывает пользователю понятное сообщение вместо «повтор N/10». Классификация специфична для backend: Claude распознаёт переполнение и лимит, Codex по умолчанию считает все ошибки временными
...
- `MAX_RETRIES = 10` — максимальное количество повторных попыток при **временной** ошибке от Claude. 10 попыток с интервалом 1 минута — суммарно до 10 минут ожидания, что покрывает большинство временных сбоев Claude API. **Постоянные ошибки** (переполнение контекста, исчерпанный лимит) распознаются backend-контрактом через `classify_permanent_error` и НЕ запускают цикл повторов — пользователю сразу уходит понятное сообщение (см. раздел «Обработка ошибок и крайние случаи» и `PermanentErrorKind` в `coding_agent_backend`)
